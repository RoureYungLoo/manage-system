...
const handleDelete = (deptId) => {
  ...
  title.value = "确认删除?"
  deletedId.value = deptId;
}

/* 确认删除 */
const onDeleteConfirm = async () => {
  const res = await deptService.deleteById(deletedId.value);
  if (res.code === 1) {
    ElMessage({
      type: "success",
      message: "删除成功"
...
const handleEdit = async (deptId) => {
  ...
  const response = await deptService.updateById(formData.value);
...
<template>
  ...
  <!-- 添加/编辑部门 -->
  <el-dialog v-model="dialogVisible.saveOrEdit" :title="title" width="500">
  ...
  <el-dialog
      ...
      width="400"
  >
    <span>删除后不可恢复, 继续删除?</span>
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="onCancel">取消</el-button>
        <el-button type="primary" @click="onDeleteConfirm">
          确定
        </el-button>
      </div>
    </template>
